...
// Search destinations with optional query, minCost, and tags
export async function searchDestinations(req: Request, res: Response) {
  try {
    const q = (req.query.q as string) || '';
    const minCost: number | undefined = req.query.minCost ? Number(req.query.minCost) : undefined;
    const tags: string[] = req.query.tags ? (req.query.tags as string).split(',') : [];
    ...
    // Filter by tags in-app
    const filtered = tags.length
      ? results.filter((r: DestinationType) => r.tags && tags.every((t) => r.tags!.includes(t)))
      : results;
